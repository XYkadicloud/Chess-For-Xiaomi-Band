...
assert(about.includes('>Chess<'));
assert(!settings.includes('走法提示'));
assert(!settings.includes('认输前确认'));
assert(!settings.includes('棋盘动画'));
assert(settings.includes('选中后自动居中'));
assert(settings.includes('棋盘大小'));
assert(manifest.router.pages['pages/purchase']);
assert(index.includes("uri:'/pages/purchase'"));
assert(index.includes('purchase_confirmed'));
assert(purchase.includes('爱发电购买'));
assert(purchase.includes('/common/aifadian_qr.jpg'));
assert(purchase.includes('2元'));
assert(purchase.includes('已购买'));
assert(purchase.includes("key:'purchase_confirmed'"));
assert(about.includes('前往付款'));
assert(about.includes('purchaseConfirmed'));
assert(about.includes("uri:'/pages/purchase'"));
