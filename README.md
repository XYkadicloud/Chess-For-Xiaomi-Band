...
1. [`docs/AGENT_HANDOFF.md`](docs/AGENT_HANDOFF.md)：仓库原有技术交接资料。
...
| `archive/` | 历史源码、诊断资料、旧 RPK 和版本归档包。历史源码已压缩保存，不作为当前构建输入。 |
| `archive/releases/` | 已生成的版本 RPK、源码归档和 SHA-256 文件。 |
| `archive/dist/` | 仓库历史上保留的 0.2.x RPK 构建产物。 |
| `docs/` | 当前交接文档和版本索引。 |
...
历史源码和诊断资料已压缩保存于 [`archive/history-20260925.tar.gz`](archive/history-20260925.tar.gz) 和 [`archive/diagnostics-20260925.tar.gz`](archive/diagnostics-20260925.tar.gz)。历史 RPK 位于 [`archive/dist/`](archive/dist/)，版本归档包位于 [`archive/exports/`](archive/exports/)。这些资料用于回退、对比和问题定位，不应直接覆盖当前 `src/`。

## Git 约定

当前可构建源码保留在仓库根目录的 `src/`，历史资料统一放在 `archive/`，交付物放在 `archive/releases/`。修改当前版本时应先备份 `src/`，完成专项校验后再提交。不要把 `node_modules/`、`build/`、临时目录或日志提交到仓库。

## 开源许可证

本项目源代码采用 **GNU General Public License v3.0（GPL-3.0）** 发布，具体条款见仓库根目录的 [`LICENSE`](LICENSE) 文件。

GPL-3.0 要求分发本项目或其衍生作品时遵守相同许可证的再发布义务，并保留版权和许可证声明。`archive/releases/` 中的 RPK 属于由本项目构建的发布产物；发布或再分发时，也应同时遵守 GPL-3.0 以及其中包含的第三方代码和资源所适用的额外条款。
